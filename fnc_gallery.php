<?php
	...
	function show_latest_public_photo(){
        ...
        $conn = new mysqli($GLOBALS["server_host"], $GLOBALS["server_user_name"], $GLOBALS["server_password"], $GLOBALS["database"]);
		$conn->set_charset("utf8");
        $stmt = $conn->prepare("SELECT id, alttext FROM vp_photos_2 WHERE id = (SELECT MAX(id) FROM vp_photos_2 WHERE privacy = ? AND deleted IS NULL)");
        echo $conn->error;
        $stmt->bind_param("i", $privacy);
        ...
        if($stmt->fetch()){
			...
			//<img src="show_public_photo.php?photo=7" alt="tekst">
            //$photo_html = '<img src="' .$GLOBALS["gallery_photo_normal_folder"] .$filename_from_db .'" alt="';
